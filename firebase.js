...
(function(){
  ...
  function startLivePull(){clearInterval(state.pullTimer);state.pullTimer=setInterval(()=>{if(shouldAutoSync())pullMerge(readLocal()?.settings?.companyKey,true).catch(e=>console.warn(e));},9000);window.addEventListener('focus',()=>{if(shouldAutoSync()&&!userIsEditing())pullMerge(readLocal()?.settings?.companyKey,true).catch(()=>{})});window.addEventListener('online',()=>{queueSync(readLocal(),400);if(!userIsEditing())pullMerge(readLocal()?.settings?.companyKey,true).catch(()=>{})});}
  function loadPatchScript(){if(state.patchScriptLoaded)return;state.patchScriptLoaded=true;try{const s=document.createElement('script');s.src='تعديل.js?v=20260509-root-sync';s.defer=true;(document.head||document.documentElement).appendChild(s);}catch(e){}}
  ...
  (function(){
    ...
    function close(){if(!mob())return;const d=document.getElementById('drawer')||document.querySelector('.drawer'),o=document.getElementById('drawerOverlay')||document.querySelector('.drawer-overlay');if(d)d.classList.remove('open');if(o)o.classList.remove('show')}
    function apply(){st();close();home()} if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>[0,80,220,500,900].forEach(ms=>setTimeout(apply,ms)),{once:true});else [0,80,220,500,900].forEach(ms=>setTimeout(apply,ms));
  })();
})();
